...
test('followPath: нос борта всегда смотрит строго вдоль траектории (без отставания)', () => {
  const { game } = boot();
  // диагональный отрезок: курс должен мгновенно встать вдоль него, без плавного доворота
  const pl = { x: 0, y: 0, ang: 0, autoPath: false, path: [{ x: 100, y: 100 }] };
  game.followPath(pl, 30, 1 / 60);
  const want = Math.atan2(100, 100);   // 45°
  assert.ok(Math.abs(pl.ang - want) < 1e-9, `курс ${pl.ang} не совпал с направлением пути ${want}`);

  // на прямоугольном маршруте на каждом кадре нос направлен на текущий целевой узел
  const route = [{ x: 50, y: 50 }, { x: 50, y: 250 }, { x: 250, y: 250 }, { x: 250, y: 50 }];
  const p2 = { x: route[0].x, y: route[0].y, ang: 0, autoPath: false, path: route.slice(1) };
  for (let i = 0; i < 5000 && p2.path.length; i++) {
    game.followPath(p2, 60, 1 / 60);
    if (p2.path.length) {
      const n = p2.path[0];
      const want2 = Math.atan2(n.y - p2.y, n.x - p2.x);
      assert.ok(Math.abs(p2.ang - want2) < 1e-9, `нос отстал от траектории на кадре ${i}`);
    }
  }
});
