...
function drawMenuScene(tm: number){
  ...
  function frame(ts: number){
    ...
    if(inMenu){
      ...
      vignette();
      drawFloaters(dt);
      drawSideClouds(ts);   // боковые облака справа — поверх поля/бортов/эффектов, но под HUD: борт «вылетает из облаков»
