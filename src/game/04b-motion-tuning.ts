...
// Reads: 04 (K, FOR).
  ...
  const MT_PLANNED_PARAMS: MtParam[] = [
    ...
    {key:'MT.GRAB_SHAPE', group:'snap_zones', category:'Зоны захвата', label:'Форма зоны захвата', target:'META', name:'GRAB_SHAPE', def:'semicircle', note:'Форма настраиваемой зоны захвата (бокс и ВПП): «semicircle» — полукруг куполом в сторону захода; «square» — квадрат со стороной 2×радиуса с центром в той же точке. Управляется теми же ручками радиус/смещение на превью.', impact:'Меняет геометрию захвата конца маршрута; для сравнения вариантов.', affectsGameplay:true, liveSafe:false, requiresReplay:true, exportable:true},
    ...
    {key:'MT.RUNWAY_GRAB_RADIUS', group:'runway_geometry', category:'Геометрия ВПП', label:'Полукруг ВПП · радиус', target:'META', name:'RUNWAY_GRAB_RADIUS', def:0, min:0, max:200, step:1, unit:'px', note:'Радиус полукруглой зоны захвата у полевого торца ВПП (px). 0 = только прямоугольник полосы. Купол смотрит в сторону захода. Перетаскивается и масштабируется прямо на превью.', impact:'Больше — шире купол захода на полосу; геометрия живёт только в Workbench.', affectsGameplay:true, liveSafe:false, requiresReplay:true, exportable:true},
    {key:'MT.RUNWAY_GRAB_OFFSET', group:'runway_geometry', category:'Геометрия ВПП', label:'Полукруг ВПП · смещение', target:'META', name:'RUNWAY_GRAB_OFFSET', def:0, min:-120, max:200, step:1, unit:'px', note:'Сдвиг центра полукруга вдоль оси захода от торца ВПП (px). Плюс — дальше в поле. Перетаскивается на превью.', impact:'Позиционирует зону захвата относительно полосы.', affectsGameplay:true, liveSafe:false, requiresReplay:true, exportable:true},
    {key:'MT.BAY_GRAB_RADIUS', group:'service_bay_geometry', category:'Геометрия боксов', label:'Полукруг бокса · радиус', target:'META', name:'BAY_GRAB_RADIUS', def:0, min:0, max:200, step:1, unit:'px', note:'Радиус полукруглой зоны захвата у ворот бокса (px). 0 = только прямоугольник бокса. Купол смотрит в сторону захода. Перетаскивается и масштабируется прямо на превью.', impact:'Больше — шире купол захода в ангар; геометрия живёт только в Workbench.', affectsGameplay:true, liveSafe:false, requiresReplay:true, exportable:true},
    {key:'MT.BAY_GRAB_OFFSET', group:'service_bay_geometry', category:'Геометрия боксов', label:'Полукруг бокса · смещение', target:'META', name:'BAY_GRAB_OFFSET', def:0, min:-120, max:200, step:1, unit:'px', note:'Сдвиг центра полукруга наружу от ворот бокса вдоль оси захода (px). Плюс — дальше в поле. Перетаскивается на превью.', impact:'Позиционирует зону захвата относительно бокса.', affectsGameplay:true, liveSafe:false, requiresReplay:true, exportable:true},
