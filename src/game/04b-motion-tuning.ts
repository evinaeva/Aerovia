...
// One fragment of the single game IIFE. Provides: MT, motion tuning UI hooks.

  type MtParam = { key:string; group:string; label:string; target:'K'|'FOR'; name:string; def:number|boolean|number[]; min?:number; max?:number; step?:number; note:string; impact:string; hardcoded?:boolean };
  ...
  const MT_PARAMS: MtParam[] = [
    ...
    {key:'K.FOG_TAXI',group:'weather',label:'Туман taxi x',target:'K',name:'FOG_TAXI',def:K.FOG_TAXI,min:.1,max:1,step:.05,note:'Множитель руления в тумане.',impact:'Ниже — туман сильнее тормозит поле.'},
    {key:'K.WEATHER_RAIN_TAXI',group:'weather',label:'Дождь taxi x',target:'K',name:'WEATHER_RAIN_TAXI',def:K.WEATHER_RAIN_TAXI,min:.1,max:1,step:.05,note:'Множитель руления в дождь.',impact:'Ниже — дождь сильнее замедляет.'},
    {key:'K.WEATHER_SNOW_TAXI',group:'weather',label:'Снег taxi x',target:'K',name:'WEATHER_SNOW_TAXI',def:K.WEATHER_SNOW_TAXI,min:.1,max:1,step:.05,note:'Множитель руления в снег.',impact:'Ниже — снег сильнее замедляет.'},
  ...
  const MT_GROUPS: Record<string,string> = {movement:'Движение',turns:'Повороты',routing:'Маршрутизация',takeoff:'Взлёт',timing:'Тайминги',service:'Обслуживание',spawn:'Поток',collisions:'Столкновения',effects:'Эффекты',events:'События',weather:'Погода',forest:'Лесной биом'};
  function mtTarget(p: MtParam){ return p.target==='FOR' ? FOR as any : K as any; }
  function mtGet(p: MtParam){ return mtTarget(p)[p.name]; }
  function mtSet(key: string, value: any){ const p=MT_PARAMS.find(x=>x.key===key); if(!p) return false; const t=mtTarget(p); if(Array.isArray(p.def)){ t[p.name]=String(value).split(',').map((x:string)=>+x.trim()).filter((x:number)=>Number.isFinite(x)); } else if(typeof p.def==='boolean') t[p.name]=!!value; else { let n=+value; if(!Number.isFinite(n)) return false; if(p.min!=null)n=Math.max(p.min,n); if(p.max!=null)n=Math.min(p.max,n); t[p.name]=n; } return true; }
  function mtSnapshot(){ const o: Record<string, any>={}; MT_PARAMS.forEach(p=>o[p.key]=mtGet(p)); return o; }
  function mtApply(values: Record<string, any>, persist=true){ Object.keys(values||{}).forEach(k=>mtSet(k, values[k])); if(persist) try{ localStorage.setItem(MT_STORE_KEY, JSON.stringify(mtSnapshot())); }catch(_){} mtRenderPanel(); }
  function mtReset(){ const o: Record<string, any>={}; MT_PARAMS.forEach(p=>o[p.key]=p.def); mtApply(o); }
  function mtLoad(){ try{ const raw=localStorage.getItem(MT_STORE_KEY); if(raw) mtApply(JSON.parse(raw), false); }catch(_){} }
  function mtPresets(){ try{return JSON.parse(localStorage.getItem(MT_PRESETS_KEY)||'{}')||{};}catch(_){return{};} }
  function mtSavePreset(name:string){ const ps=mtPresets(); ps[name]=mtSnapshot(); localStorage.setItem(MT_PRESETS_KEY, JSON.stringify(ps)); mtRenderPanel(); }
  function mtApplyPreset(name:string){ const ps=mtPresets(); if(ps[name]) mtApply(ps[name]); }
  function mtExport(){ return JSON.stringify({version:1, exportedAt:new Date().toISOString(), values:mtSnapshot()}, null, 2); }
  function mtImportText(text:string){ const o=JSON.parse(text); mtApply(o.values||o); }
  const MT = { params:MT_PARAMS, groups:MT_GROUPS, snapshot:mtSnapshot, apply:mtApply, reset:mtReset, export:mtExport, importText:mtImportText, savePreset:mtSavePreset, applyPreset:mtApplyPreset };
  function mtRenderPanel(){ const root=document.getElementById('motionTuningPanel'); if(!root || root.classList.contains('hidden')) return; const ps=mtPresets(); root.innerHTML='<div class="mt-head"><b>Motion Tuning</b><button id="mtClose">×</button></div><div class="mt-actions"><button id="mtReset">Reset</button><button id="mtExport">Export JSON</button><button id="mtImport">Import</button><input id="mtPresetName" placeholder="Preset name"><button id="mtSavePreset">Save</button><select id="mtPreset"><option value="">Preset…</option>'+Object.keys(ps).map(n=>'<option>'+n+'</option>').join('')+'</select></div>'+Object.keys(MT_GROUPS).map(g=>'<details open><summary>'+MT_GROUPS[g]+'</summary>'+MT_PARAMS.filter(p=>p.group===g).map(p=>'<label class="mt-row"><span><b>'+p.label+'</b><em>'+p.key+' · '+p.impact+'</em></span><input type="range" data-mt="'+p.key+'" min="'+p.min+'" max="'+p.max+'" step="'+p.step+'" value="'+mtGet(p)+'"><input class="mt-num" data-mt="'+p.key+'" type="number" step="'+p.step+'" value="'+mtGet(p)+'"></label>').join('')+'</details>').join('');
    ...
    root.querySelector('#mtExport')!.addEventListener('click',()=>{ const a=document.createElement('a'); a.href=URL.createObjectURL(new Blob([mtExport()],{type:'application/json'})); a.download='motion-tuning.json'; a.click(); setTimeout(()=>URL.revokeObjectURL(a.href),5000); });
    root.querySelector('#mtImport')!.addEventListener('click',()=>{ const txt=prompt('Вставьте JSON Motion Tuning'); if(txt) try{mtImportText(txt);}catch(e){alert('JSON не импортирован: '+e);} });
    root.querySelector('#mtSavePreset')!.addEventListener('click',()=>{ const i=root.querySelector('#mtPresetName') as HTMLInputElement; if(i.value.trim()) mtSavePreset(i.value.trim()); });
    root.querySelector('#mtPreset')!.addEventListener('change',(e:any)=>{ if(e.target.value) mtApplyPreset(e.target.value); });
    root.querySelectorAll('[data-mt]').forEach((el:any)=>el.addEventListener('input',(e:any)=>{ mtSet(e.target.dataset.mt, e.target.value); try{localStorage.setItem(MT_STORE_KEY, JSON.stringify(mtSnapshot()));}catch(_){} root.querySelectorAll('[data-mt="'+e.target.dataset.mt+'"]').forEach((x:any)=>{ if(x!==e.target)x.value=e.target.value; }); })); }
  function mtOpenPanel(){ let root=document.getElementById('motionTuningPanel'); if(!root){ root=document.createElement('div'); root.id='motionTuningPanel'; root.className='motion-tuning'; document.getElementById('stage')!.appendChild(root); } root.classList.remove('hidden'); mtRenderPanel(); }
  mtLoad();
