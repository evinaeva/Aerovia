...
// Reads: 04 (K, FOR).
  ...
  const MT_PLANNED_PARAMS: MtParam[] = [
    ...
    {key:'MT.DEBUG_MOTION_POINTS', group:'flight_overlay', category:'Отладочные слои', label:'Показать точки', target:'META', name:'DEBUG_MOTION_POINTS', def:false, note:'Включить отображение перетаскиваемых точек на превью: место касания, место отрыва и начало выравнивания на ВПП, а также точку подъезда к ангару. Точки можно двигать мышью — они меняют соответствующие K.RW_* и K.BAY_APPROACH_DIST.', impact:'Только визуализация — на геймплей влияют связанные параметры K.', visualsOnly:true, liveSafe:true, exportable:true, debugOnly:true},
  ...
  const MT_PARAMS: MtParam[] = [
    ...
    // ── размер техники: один масштаб борта → ШИРИНА ВПП и ангар на всех картах ─
    // Длина ВПП к размеру борта НЕ привязана — управляется K.RUNWAY_R независимо.
    {key:'K.PLANE_SCALE',  group:'aircraft_scale', label:'Размер борта (общий)', target:'K', name:'PLANE_SCALE',  def:K.PLANE_SCALE,  min:.5, max:2.5, step:.05, note:'Общий масштаб техники — крупность борта. Из него выводятся ШИРИНА ВПП (rh = борт × K.RUNWAY_RATIO) и размер ангара. Длина ВПП от масштаба борта не зависит — её задаёт K.RUNWAY_R.', impact:'Больше — крупнее борт, шире ВПП и больше ангары; длина ВПП не меняется.'},
    {key:'K.RUNWAY_RATIO', group:'aircraft_scale', label:'Ширина ВПП ÷ борт',    target:'K', name:'RUNWAY_RATIO', def:K.RUNWAY_RATIO, min:.8, max:3,   step:.01, note:'Ширина полосы = длина борта × коэф. Длина ВПП этим коэффициентом не управляется — только поперечный габарит.', impact:'Больше — шире полосы (борт занимает меньшую долю ширины); длина ВПП не меняется.'},
    ...
    // ── геометрия ВПП ────────────────────────────────────────────────────────
    {key:'K.RUNWAY_R', group:'runway_geometry', label:'Длина ВПП (правый край ×W)', target:'K', name:'RUNWAY_R', def:K.RUNWAY_R, min:.72, max:.93, step:.01, note:'Правый (небесный) торец ВПП как доля ширины экрана. Левый край фиксирован у кромки апрона (≈0.63W), поэтому ползунок управляет длиной: длина ≈(RUNWAY_R − 0.63)×W. Зона ожидания бортов сдвигается пропорционально.', impact:'Больше — длиннее ВПП и дальше зона ожидания.'},
