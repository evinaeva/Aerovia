...
// Reads: 04 (K, FOR).
  ...
  const MT_PLANNED_PARAMS: MtParam[] = [
    ...
    {key:'MT.RUNWAY_HIT_PADDING', group:'runway_geometry', category:'Геометрия ВПП', label:'Поле касания ВПП', target:'META', name:'RUNWAY_HIT_PADDING', def:0, min:0, max:120, step:1, unit:'px', note:'Запас вокруг ВПП, в который «прилипает» конец маршрута (px). 0 = ровно прямоугольник полосы. Зону рисует слой «Зоны захвата ВПП».', impact:'Больше — легче завести борт на полосу; геометрия живёт только в Workbench.', affectsGameplay:true, liveSafe:false, requiresReplay:true, exportable:true},
    {key:'MT.BAY_HIT_PADDING', group:'service_bay_geometry', category:'Геометрия боксов', label:'Поле касания бокса', target:'META', name:'BAY_HIT_PADDING', def:0, min:0, max:120, step:1, unit:'px', note:'Запас вокруг бокса, в который «прилипает» конец маршрута (px). 0 = ровно прямоугольник бокса. Зону рисует слой «Зоны захвата боксов».', impact:'Больше — легче завести борт в ангар; геометрия живёт только в Workbench.', affectsGameplay:true, liveSafe:false, requiresReplay:true, exportable:true},
    {key:'MT.RUNWAY_GRAB_SHAPE', group:'runway_geometry', category:'Геометрия ВПП', label:'Форма зоны ВПП', target:'META', name:'RUNWAY_GRAB_SHAPE', def:'semicircle', note:'Форма зон захвата ВПП (обе стороны): «semicircle» — полукруг куполом в сторону захода; «square» — квадрат со стороной 2×радиуса с центром в той же точке.', impact:'Меняет геометрию захвата конца маршрута у ВПП; для сравнения вариантов.', affectsGameplay:true, liveSafe:false, requiresReplay:true, exportable:true},
    {key:'MT.RUNWAY_LAND_GRAB_RADIUS', group:'runway_geometry', category:'Геометрия ВПП', label:'Посадка · радиус', target:'META', name:'RUNWAY_LAND_GRAB_RADIUS', def:0, min:0, max:200, step:1, unit:'px', note:'Радиус зоны захвата у посадочного (правого, со стороны неба) торца ВПП (px). 0 = только прямоугольник полосы. Перетаскивается и масштабируется на превью.', impact:'Больше — шире зона захода на посадку; геометрия живёт только в Workbench.', affectsGameplay:true, liveSafe:false, requiresReplay:true, exportable:true},
    {key:'MT.RUNWAY_LAND_GRAB_OFFSET', group:'runway_geometry', category:'Геометрия ВПП', label:'Посадка · смещение', target:'META', name:'RUNWAY_LAND_GRAB_OFFSET', def:0, min:-120, max:200, step:1, unit:'px', note:'Сдвиг центра посадочной зоны вдоль оси захода от правого торца ВПП (px). Плюс — дальше в небо. Перетаскивается на превью.', impact:'Позиционирует посадочную зону захвата.', affectsGameplay:true, liveSafe:false, requiresReplay:true, exportable:true},
    {key:'MT.RUNWAY_TAKEOFF_GRAB_RADIUS', group:'runway_geometry', category:'Геометрия ВПП', label:'Взлёт · радиус', target:'META', name:'RUNWAY_TAKEOFF_GRAB_RADIUS', def:0, min:0, max:200, step:1, unit:'px', note:'Радиус зоны захвата у взлётного (левого, со стороны апрона) торца ВПП (px). 0 = только прямоугольник полосы. Перетаскивается и масштабируется на превью.', impact:'Больше — шире зона захода на взлёт; геометрия живёт только в Workbench.', affectsGameplay:true, liveSafe:false, requiresReplay:true, exportable:true},
    {key:'MT.RUNWAY_TAKEOFF_GRAB_OFFSET', group:'runway_geometry', category:'Геометрия ВПП', label:'Взлёт · смещение', target:'META', name:'RUNWAY_TAKEOFF_GRAB_OFFSET', def:0, min:-120, max:200, step:1, unit:'px', note:'Сдвиг центра взлётной зоны вдоль оси захода от левого торца ВПП (px). Плюс — дальше в поле/апрон. Перетаскивается на превью.', impact:'Позиционирует взлётную зону захвата.', affectsGameplay:true, liveSafe:false, requiresReplay:true, exportable:true},
    {key:'MT.BAY_GRAB_SHAPE', group:'service_bay_geometry', category:'Геометрия боксов', label:'Форма зоны бокса', target:'META', name:'BAY_GRAB_SHAPE', def:'semicircle', note:'Форма зоны захвата бокса: «semicircle» — полукруг куполом в сторону захода; «square» — квадрат со стороной 2×радиуса с центром в той же точке.', impact:'Меняет геометрию захвата конца маршрута у бокса; для сравнения вариантов.', affectsGameplay:true, liveSafe:false, requiresReplay:true, exportable:true},
    {key:'MT.BAY_GRAB_RADIUS', group:'service_bay_geometry', category:'Геометрия боксов', label:'Бокс · радиус', target:'META', name:'BAY_GRAB_RADIUS', def:0, min:0, max:200, step:1, unit:'px', note:'Радиус зоны захвата у ворот бокса (px). 0 = только прямоугольник бокса. Перетаскивается и масштабируется на превью.', impact:'Больше — шире зона захода в ангар; геометрия живёт только в Workbench.', affectsGameplay:true, liveSafe:false, requiresReplay:true, exportable:true},
    {key:'MT.BAY_GRAB_OFFSET', group:'service_bay_geometry', category:'Геометрия боксов', label:'Бокс · смещение', target:'META', name:'BAY_GRAB_OFFSET', def:0, min:-120, max:200, step:1, unit:'px', note:'Сдвиг центра зоны наружу от ворот бокса вдоль оси захода (px). Плюс — дальше в поле. Перетаскивается на превью.', impact:'Позиционирует зону захвата бокса.', affectsGameplay:true, liveSafe:false, requiresReplay:true, exportable:true},
    {key:'MT.DEBUG_RUNWAY_SNAP_ZONES', group:'debug_overlays', category:'Отладочные слои', label:'Зоны захвата ВПП', target:'META', name:'DEBUG_RUNWAY_SNAP_ZONES', def:false, note:'Рисует поверх поля зону захвата каждой открытой ВПП (полотно полосы + «Поле касания ВПП»).', impact:'Только отрисовка для настройки геометрии; на геймплей не влияет.', visualsOnly:true, liveSafe:true, exportable:true, debugOnly:true},
    ...
    {key:'MT.DEBUG_MOTION_POINTS', group:'flight_overlay', category:'Отладочные слои', label:'Точки ВПП/ангара', target:'META', name:'DEBUG_MOTION_POINTS', def:false, note:'Рисует на превью и в тестовой игре перетаскиваемые точки: касание, отрыв и начало выравнивания на ВПП, а также точку подъезда к ангару. Тяни их вдоль полосы/оси ворот, чтобы задать K.RW_TOUCHDOWN_OFF / K.RW_LIFTOFF_OFF / K.RW_ALIGN_OFF / K.BAY_APPROACH_DIST.', impact:'Сам слой — только отрисовка/перетаскивание; на геймплей влияют связанные параметры K.', visualsOnly:true, liveSafe:true, exportable:true, debugOnly:true},
  ...
  const MT_PARAMS: MtParam[] = [
    ...
    // ── маршрутизация ─────────────────────────────────────────────────────────
    {key:'K.ARRIVE', group:'routing', label:'Захват точки маршрута',          target:'K',  name:'ARRIVE',       def:K.ARRIVE,       min:2,   max:60,  step:1,    note:'Минимальный порог достижения waypoint.',                    impact:'Больше — плавнее, но менее точное следование линии.'},
    ...
    {key:'K.TWO_SVC_CHANCE',group:'events', label:'Шанс двух услуг',         target:'K',  name:'TWO_SVC_CHANCE',def:K.TWO_SVC_CHANCE,min:0, max:1,   step:.01,  note:'Вероятность двух запросов при спавне.',                     impact:'Выше — длиннее маршруты и больше чек.'},
    // ── поток (spawn) ─────────────────────────────────────────────────────────
    ...
    {key:'K.BAY_APPROACH_DIST', group:'bay_nav', label:'Точка подъезда к ангару', target:'K', name:'BAY_APPROACH_DIST', def:K.BAY_APPROACH_DIST, min:0, max:300, step:2, unit:'px', note:'Дистанция точки подъезда от ворот ангара наружу по оси захода (px·ui). Борт доезжает до неё, центрируется по оси ворот и заезжает прямо. 0 = заезжает сразу при касании бокса. Перетаскивается на превью (слой «Точки ВПП/ангара»).', impact:'Определяет, с какого места перед ангаром борт центрирует траекторию и заезжает прямо.'},
    // ── настраиваемые точки ВПП (перетаскиваются на превью) ───────────────────
    {key:'K.RW_TOUCHDOWN_OFF', group:'landing',  label:'Точка касания (сдвиг)',       target:'K', name:'RW_TOUCHDOWN_OFF', def:K.RW_TOUCHDOWN_OFF, min:-200, max:400, step:2, unit:'px', note:'Сдвиг точки приземления вдоль ВПП от штатной (полевой торец + корпус). + = дальше в небо (касание раньше и дальше от апрона), − = ближе к апрону. Перетаскивается на превью (слой «Точки ВПП/ангара»).', impact:'Определяет, в каком месте ВПП борт касается земли при посадке.'},
    {key:'K.RW_LIFTOFF_OFF',   group:'takeoff',  label:'Точка отрыва (сдвиг)',        target:'K', name:'RW_LIFTOFF_OFF',   def:K.RW_LIFTOFF_OFF,   min:-300, max:400, step:2, unit:'px', note:'Сдвиг точки отрыва от земли на взлёте вдоль ВПП от небесного торца. + = отрыв дальше в небо (позже), − = раньше. Двигает и точку роста масштаба. Перетаскивается на превью.', impact:'Определяет, в каком месте борт отрывается от земли при взлёте.'},
    {key:'K.RW_ALIGN_OFF',     group:'approach', label:'Начало выравнивания (сдвиг)', target:'K', name:'RW_ALIGN_OFF',     def:K.RW_ALIGN_OFF,     min:-300, max:400, step:2, unit:'px', note:'Точка, с которой борт выравнивается по оси ВПП при заходе. Сдвиг от небесного торца ВПП: + = выравнивание раньше (дальше в небе), − = позже (ближе к апрону). Перетаскивается на превью.', impact:'Определяет, с какого места заход «прилипает» к осевой линии полосы.'},
