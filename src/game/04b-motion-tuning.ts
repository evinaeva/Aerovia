...
// Reads: 04 (K, FOR).
  ...
  const MT_PLANNED_PARAMS: MtParam[] = [
    ...
    {key:'MT.RUNWAY_LAND_GRAB_RADIUS', group:'runway_geometry', category:'Геометрия ВПП', label:'Посадка · радиус', target:'META', name:'RUNWAY_LAND_GRAB_RADIUS', def:24, min:0, max:200, step:1, unit:'px', note:'Радиус зоны захвата у посадочного (правого, со стороны неба) торца ВПП. 0 = только прямоугольник полосы. Перетаскивается и масштабируется на превью.', impact:'Больше — шире зона захода на посадку; геометрия живёт только в Workbench.', affectsGameplay:true, liveSafe:false, requiresReplay:true, exportable:true},
    {key:'MT.RUNWAY_LAND_GRAB_OFFSET', group:'runway_geometry', category:'Геометрия ВПП', label:'Посадка · смещение', target:'META', name:'RUNWAY_LAND_GRAB_OFFSET', def:-13, min:-120, max:200, step:1, unit:'px', note:'Сдвиг центра посадочной зоны вдоль оси захода от правого торца ВПП. Плюс — дальше в небо. Перетаскивается на превью.', impact:'Позиционирует посадочную зону захвата.', affectsGameplay:true, liveSafe:false, requiresReplay:true, exportable:true},
    {key:'MT.RUNWAY_TAKEOFF_GRAB_RADIUS', group:'runway_geometry', category:'Геометрия ВПП', label:'Взлёт · радиус', target:'META', name:'RUNWAY_TAKEOFF_GRAB_RADIUS', def:23, min:0, max:200, step:1, unit:'px', note:'Радиус зоны захвата у взлётного (левого, со стороны апрона) торца ВПП. 0 = только прямоугольник полосы. Перетаскивается и масштабируется на превью.', impact:'Больше — шире зона захода на взлёт; геометрия живёт только в Workbench.', affectsGameplay:true, liveSafe:false, requiresReplay:true, exportable:true},
    {key:'MT.RUNWAY_TAKEOFF_GRAB_OFFSET', group:'runway_geometry', category:'Геометрия ВПП', label:'Взлёт · смещение', target:'META', name:'RUNWAY_TAKEOFF_GRAB_OFFSET', def:-12, min:-120, max:200, step:1, unit:'px', note:'Сдвиг центра взлётной зоны вдоль оси захода от левого торца ВПП. Плюс — дальше в поле/апрон. Перетаскивается на превью.', impact:'Позиционирует взлётную зону захвата.', affectsGameplay:true, liveSafe:false, requiresReplay:true, exportable:true},
