...
// Reads: 04 (K, FOR).
  ...
  const MT_PLANNED_PARAMS: MtParam[] = [
    ...
    {key:'MT.RUNWAY_HIT_PADDING', group:'runway_geometry', category:'Геометрия ВПП', label:'Поле касания ВПП', target:'META', name:'RUNWAY_HIT_PADDING', def:0, min:0, max:120, step:1, unit:'px', note:'Запас вокруг ВПП, в который «прилипает» конец маршрута. 0 = ровно прямоугольник полосы. Зону рисует слой «Зоны захвата ВПП».', impact:'Больше — легче завести борт на полосу; геометрия живёт только в Workbench.', affectsGameplay:true, liveSafe:false, requiresReplay:true, exportable:true},
    {key:'MT.BAY_HIT_PADDING', group:'service_bay_geometry', category:'Геометрия боксов', label:'Поле касания бокса', target:'META', name:'BAY_HIT_PADDING', def:0, min:0, max:120, step:1, unit:'px', note:'Запас вокруг бокса, в который «прилипает» конец маршрута. 0 = ровно прямоугольник бокса. Зону рисует слой «Зоны захвата боксов».', impact:'Больше — легче завести борт в ангар; геометрия живёт только в Workbench.', affectsGameplay:true, liveSafe:false, requiresReplay:true, exportable:true},
    {key:'MT.RUNWAY_GRAB_SHAPE', group:'runway_geometry', category:'Геометрия ВПП', label:'Форма зоны ВПП', target:'META', name:'RUNWAY_GRAB_SHAPE', def:'semicircle', note:'Форма зон захвата ВПП (обе стороны): «полукруг» — куполом в сторону захода; «квадрат» — со стороной в два радиуса и центром в той же точке.', impact:'Меняет геометрию захвата конца маршрута у ВПП; для сравнения вариантов.', affectsGameplay:true, liveSafe:false, requiresReplay:true, exportable:true},
    {key:'MT.RUNWAY_LAND_GRAB_RADIUS', group:'runway_geometry', category:'Геометрия ВПП', label:'Посадка · радиус', target:'META', name:'RUNWAY_LAND_GRAB_RADIUS', def:0, min:0, max:200, step:1, unit:'px', note:'Радиус зоны захвата у посадочного (правого, со стороны неба) торца ВПП. 0 = только прямоугольник полосы. Перетаскивается и масштабируется на превью.', impact:'Больше — шире зона захода на посадку; геометрия живёт только в Workbench.', affectsGameplay:true, liveSafe:false, requiresReplay:true, exportable:true},
    {key:'MT.RUNWAY_LAND_GRAB_OFFSET', group:'runway_geometry', category:'Геометрия ВПП', label:'Посадка · смещение', target:'META', name:'RUNWAY_LAND_GRAB_OFFSET', def:0, min:-120, max:200, step:1, unit:'px', note:'Сдвиг центра посадочной зоны вдоль оси захода от правого торца ВПП. Плюс — дальше в небо. Перетаскивается на превью.', impact:'Позиционирует посадочную зону захвата.', affectsGameplay:true, liveSafe:false, requiresReplay:true, exportable:true},
    {key:'MT.RUNWAY_TAKEOFF_GRAB_RADIUS', group:'runway_geometry', category:'Геометрия ВПП', label:'Взлёт · радиус', target:'META', name:'RUNWAY_TAKEOFF_GRAB_RADIUS', def:0, min:0, max:200, step:1, unit:'px', note:'Радиус зоны захвата у взлётного (левого, со стороны апрона) торца ВПП. 0 = только прямоугольник полосы. Перетаскивается и масштабируется на превью.', impact:'Больше — шире зона захода на взлёт; геометрия живёт только в Workbench.', affectsGameplay:true, liveSafe:false, requiresReplay:true, exportable:true},
    {key:'MT.RUNWAY_TAKEOFF_GRAB_OFFSET', group:'runway_geometry', category:'Геометрия ВПП', label:'Взлёт · смещение', target:'META', name:'RUNWAY_TAKEOFF_GRAB_OFFSET', def:0, min:-120, max:200, step:1, unit:'px', note:'Сдвиг центра взлётной зоны вдоль оси захода от левого торца ВПП. Плюс — дальше в поле/апрон. Перетаскивается на превью.', impact:'Позиционирует взлётную зону захвата.', affectsGameplay:true, liveSafe:false, requiresReplay:true, exportable:true},
    ...
    {key:'MT.BAY_GRAB_RADIUS', group:'service_bay_geometry', category:'Геометрия боксов', label:'Бокс · радиус', target:'META', name:'BAY_GRAB_RADIUS', def:0, min:0, max:200, step:1, unit:'px', note:'Радиус зоны захвата у ворот бокса. 0 = только прямоугольник бокса. Перетаскивается и масштабируется на превью.', impact:'Больше — шире зона захода в ангар; геометрия живёт только в Workbench.', affectsGameplay:true, liveSafe:false, requiresReplay:true, exportable:true},
