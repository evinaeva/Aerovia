...
// Reads: 08 (land, touchdown, depart, killAir/killCrash, spawnPlane, dist, selected…); 04 (K, LV, pace*, dayCycle, weatherTaxiMult); 06 (planes, runways, bays, field, gameTime, lives…); 07 (SND, HAP); 09b (nearMiss, pulseFx, updateTutorial); 12 (ACH); 10 (endLevel).
  ...
  function followPath(pl: any, spd: number, dt: number){
    ...
    // Захват ПРОМЕЖУТОЧНОЙ точки. Раньше он был фиксирован в turnR·0.6 (≈45–67 px) и
    // съедал по 4–5 нарисованных узлов разом — борт заметно срезал даже плавные круги и
    // «ехал не там, где проложен путь». Теперь захват АДАПТИВНЫЙ по геометрии маршрута:
    //   • на прямых и пологих дугах (радиус кривизны ≥ радиуса разворота) захват = базовый
    //     K.PATH_CAPTURE — борт идёт почти вплотную к линии;
    //   • на изломах круче, чем борт физически может завернуть (радиус кривизны < turnR),
    //     захват плавно растёт к ~0.78·turnR — борт срезает дугу ЗАРАНЕЕ и проходит угол
    //     по своему минимальному радиусу, без заноса наружу.
    // Чтобы вписать поворот, дугу надо начать тем раньше, чем он круче (геометрия касания).
    // Конечную точку и прямой авто-маршрут по-прежнему доезжаем вплотную (K.ARRIVE):
    // именно от неё зависит rectHit ВПП/бокса — въезд под взлёт/в ангар, заход на посадку.
    let capture: number;
    if(pl.autoPath || isLast){
      capture = K.ARRIVE;
    } else {
      // радиус кривизны пути ~0.7·turnR впереди: длина дуги / суммарный поворот курса
      const look = turnR*0.7;
      let acc=0, k=0;
      ...
      const Rpath = Math.abs(th) > 0.05 ? acc/Math.abs(th) : Infinity;
      const tightness = turnR / Math.max(8, Rpath);      // 1 = ровно на пределе поворотливости
      const over = Math.max(0, tightness - 1.2);         // запас «круче, чем можем» (порог 1.2)
      // Базовый захват не должен превышать прежний turnR·0.6: на МЕДЛЕННОМ ходу (руление в
      // туман/непогоду, taxiSpeed падает в ~10 раз) радиус разворота крошечный, борт может
      // следовать вплотную — фикс. пол K.PATH_CAPTURE·ui тогда оказался бы БОЛЬШЕ шага точек
      // (~12 px) и борт срезал бы углы сильнее прежнего. Привязываем пол к turnR на малой скорости.
      const floor = Math.min(K.PATH_CAPTURE*ui, turnR*0.6);
      capture = Math.max(floor, Math.min(turnR*0.78, floor + over*2*turnR));
    }
