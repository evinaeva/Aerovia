...
// Reads: 03 (I18N, t, lang, DEFAULT_LANG); 06 (state: gameTime, served, runways, save, combo, debug, weather, effects, levelIdx).
  ...
  const K = {
    TURN: 7.55,           // rad/sec поворот в полёте
    SPEED_AIR: 52,        // скорость захода на посадку
    SPEED_TAXI: 71,       // скорость руления по земле
    SPEED_TAKEOFF: 105,   // максимальная скорость разбега по ВПП
    TAKEOFF_INIT_SPEED: 5,   // начальная скорость перед разбегом (px/s; 0 = с места)
    TAKEOFF_ACCEL: 120,      // ускорение при разбеге (px/s²)
    SPEED_CLIMB: 145,        // максимальная скорость в наборе высоты после отрыва (px/s)
    TAKEOFF_CLIMB_ACCEL: 220,// ускорение после отрыва от земли (px/s²)
    LAND_ROLLOUT_DECEL: 60,  // замедление при пробеге после касания (px/s²)
    ...
    PLANE_SKY_SCALE: 1.6, // масштаб борта в небе
    PLANE_GND_SCALE: 1.35,// масштаб на земле/поле
    // ── размер техники: борт задаёт ШИРИНУ ВПП и ангар на ВСЕХ картах ───────────
    // PLANE_SCALE — общий масштаб «крупности» борта (×1). Длина борта = 31·SZ·ui;
    // из неё через RUNWAY_RATIO выводится ШИРИНА (поперечный габарит) ВПП, а через
    // HANGAR_RATIO — сторона квадратного ангара. ДЛИНА ВПП от размера борта не зависит
    // — управляется RUNWAY_R (доля ширины экрана, независимый параметр).
    PLANE_SCALE: 1,       // общий масштаб борта (крупность техники)
    RUNWAY_RATIO: 1.23,   // ШИРИНА ВПП / длина борта (≈ размах + запас); длина — отдельно через RUNWAY_R
    RUNWAY_R: 0.84,       // правый (небесный) край ВПП, доля ширины экрана; длина ≈(RUNWAY_R−0.63)×W
    HANGAR_RATIO: 1.8,    // сторона квадратного ангара / длина борта
    ...
    TAKEOFF_LIFT_DIST: 180,
    LAND_BUMP_MS: 240,    // длительность визуального «толчка» при касании, мс
    APPROACH_SPEED_MULT: 1.0,  // скорость финального захода = SPEED_AIR × это
    LAND_ALIGN_SPEED: 30,       // lerp-скорость довыравнивания по оси ВПП
    ...
    RW_TOUCHDOWN_OFF:  0,  // сдвиг точки касания вдоль ВПП от штатной (stopX+корпус). + = дальше в небо (касание раньше/дальше от апрона)
    RW_LIFTOFF_OFF:    0,  // сдвиг точки отрыва на взлёте от торца ВПП (exitX). + = отрыв дальше в небо (позже)
    RW_ALIGN_OFF:      0,  // сдвиг точки начала выравнивания по оси ВПП от небесного торца (rwR). + = выравнивание раньше (дальше в небо)
