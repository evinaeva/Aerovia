...
// Reads: 03 (I18N, t, lang, DEFAULT_LANG); 06 (state: gameTime, served, runways, save, combo, debug, weather, effects, levelIdx).
  ...
  const K = {
    ...
    LAND_ROLLOUT: 50,
    // множитель посадочного докатывания: борт при пробеге тормозит (LAND_ROLLOUT_DECEL)
    // не ниже этой доли скорости захода и докатывается на ней до остановки.
    LAND_ROLLOUT_MIN: 0.1,
    ...
    BAY_TURN_THRESHOLD: 0.04, // порог разворота в боксе (рад): при выезде борт крутится на месте, пока угол до направления «наружу» больше — ниже считаем доворот завершённым
    // ── превью маршрута (фосфорная линия; чисто визуально) ──────────────────────
    ROUTE_WIDTH: 4.8,      // толщина линии маршрута (× ui; на телефоне пол 0.4× для читаемости)
    ROUTE_ALPHA: 0.6,      // яркость (непрозрачность) линии; выбранный борт — +0.35
