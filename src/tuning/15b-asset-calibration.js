  ...
  (function () {
    ...
    const KIND_RU  = { apron:'Апрон', hangar:'Ангар', runway:'ВПП', plane:'Самолёт', runwayConnector:'Соединитель ВПП', hud:'HUD', background:'Фон', sky:'Небо' };
    const LAYER_RU = { background:'Фон', apron:'Апрон', structures:'Строения', entities:'Объекты', routes:'Маршруты', vfx:'Эффекты', hud:'HUD' };
    const POINT_KIND_RU = { anchor:'Якорь', entrance:'Вход', exit:'Выход', insideStop:'Внутренний стоп', snap:'Примагничивание', landingEntry:'Вход на посадку', touchdown:'Приземление', runwayStop:'Стоп ВПП', takeoffStart:'Начало разбега', liftOff:'Отрыв', centerlineStart:'Ось: начало', centerlineEnd:'Ось: конец', apronConnection:'Стык: апрон', runwayConnection:'Стык: ВПП', nose:'Нос', tail:'Хвост', shadowOffset:'Смещение тени', hudTextSlot:'Слот текста', hudIconSlot:'Слот иконки', pauseButton:'Кнопка паузы' };
    const RECT_KIND_RU  = { visualBounds:'Визуальная рамка', collisionBounds:'Коллизия', hitArea:'Зона касания', contentSafeArea:'Безопасная зона', gameplayArea:'Игровая зона', textSlot:'Слот текста', iconSlot:'Слот иконки', decorativeOnly:'Только декор' };
    const POINT_KIND_DESC = { anchor:'Пивот ассета; от него отсчитываются координаты в runtime.', entrance:'Самолёт подъезжает сюда перед заходом в ангар.', exit:'Отсюда самолёт выезжает из ангара.', insideStop:'Место финальной остановки самолёта внутри ангара.', snap:'Зона притяжения: самолёт в радиусе автоматически прилипает.', landingEntry:'Начало глиссады захода на посадку (опционально).', touchdown:'Место касания ВПП при посадке.', runwayStop:'Конец пробега после посадки.', takeoffStart:'Точка начала разбега перед взлётом.', liftOff:'Точка отрыва от ВПП.', centerlineStart:'Начало осевой линии ВПП.', centerlineEnd:'Конец осевой линии ВПП.', apronConnection:'Стык соединителя с апроном.', runwayConnection:'Стык соединителя с ВПП.', nose:'Нос самолёта — для коллизий и ориентации.', tail:'Хвост самолёта (опционально).', shadowOffset:'Смещение тени от якоря.', hudTextSlot:'Центр текстового поля HUD.', hudIconSlot:'Центр иконки HUD.', pauseButton:'Кнопка паузы.' };
    const RECT_KIND_DESC  = { visualBounds:'Полный визуальный размер спрайта; обязателен.', collisionBounds:'Упрощённый прямоугольник для физики столкновений.', hitArea:'Зона нажатия — обычно чуть больше спрайта.', contentSafeArea:'Область апрона, куда не заходят ключевые элементы.', gameplayArea:'Активная зона апрона для манёвров самолётов.', textSlot:'Место для текстовой метки (HUD).', iconSlot:'Место для иконки (HUD).', decorativeOnly:'Только декор — без коллизий и событий.' };
    const kindRu  = (k) => KIND_RU[k]         || k;
    const layerRu = (k) => LAYER_RU[k]        || k;
    const ptRu    = (k) => POINT_KIND_RU[k]   || k;
    const rcRu    = (k) => RECT_KIND_RU[k]    || k;
    const anyRu   = (type, k) => type === 'rect' ? rcRu(k)          : ptRu(k);
    const anyDesc = (type, k) => type === 'rect' ? (RECT_KIND_DESC[k]  || '') : (POINT_KIND_DESC[k] || '');
    const SPECS = {
    ...
    function render() {
      ...
      root.innerHTML = `<div class="ac-grid">
        <div class="ac-form">
          <div class="ac-row"><label>${tip('ID ассета', FIELD_HINTS.id)}<input data-field="id" value="${escHtml(a.id)}" placeholder="hangar_fuel_v1"></label><label>${tip('Тип ассета', FIELD_HINTS.kind)}<select data-field="kind">${ASSET_KINDS.map(k=>`<option value="${k}" ${a.kind===k?'selected':''}>${kindRu(k)}</option>`).join('')}</select></label></div>
          ...
          <div class="ac-row"><label>${tip('Смещение поворота', FIELD_HINTS.rotationOffset)}<input type="number" data-field="rotationOffset" value="${escHtml(a.rotationOffset)}"></label><label>${tip('Слой', FIELD_HINTS.layer)}<select data-field="layer"><option value=""></option>${LAYERS.map(k=>`<option value="${k}" ${a.layer===k?'selected':''}>${layerRu(k)}</option>`).join('')}</select></label></div>
        ...
        <div class="ac-side"><details class="ac-howto"><summary>Как пользоваться</summary><ol><li>Выберите <b>тип ассета</b> (Ангар, ВПП, Самолёт…), заполните ID и загрузите PNG.</li><li>Нажмите нужный пункт из списка <em>«Обязательно»</em> — он подсветится синим (это активный инструмент).</li><li><b>Точка:</b> кликните на холсте. <b>Прямоугольник:</b> зажмите и тяните.</li><li>Числа X/Y/W/H нормализованы: 0 = левый/верхний край, 1 = правый/нижний.</li><li>Все пункты зелёные → жмите «Скачать выбранный» или «Применить к превью».</li></ol></details><h4>Обязательно для ${escHtml(kindRu(a.kind))}</h4><div class="ac-checks">${[...sp.rects.map(k=>['rect',k]), ...(sp.anyRect?[['rectAny',sp.anyRect.join(' or ')]]:[]), ...sp.points.map(k=>['point',k])].map(([t,k])=>{ const ok=t==='rectAny'?a.rects.some(r=>sp.anyRect.includes(r.kind)):t==='rect'?a.rects.some(r=>r.kind===k):a.points.some(p=>p.kind===k); const kname=String(k).split(' ')[0]; const ktype=t==='point'?'point':'rect'; return `<button class="ac-check ${ok?'ok':'missing'} ${state.selectedKind===k?'sel':''}" data-select-type="${ktype}" data-kind="${escHtml(kname)}" title="${escHtml(anyDesc(ktype,kname))}">${ok?'✓':'○'} <span class="ac-check-ru">${escHtml(anyRu(ktype,kname))}</span><code class="ac-check-code">${escHtml(kname)}</code></button>`; }).join('')}</div>
          <details open><summary>Опционально/рекомендуется</summary><div class="ac-checks">${[...(sp.optionalRects||[]).map(k=>['rect',k]), ...(sp.optionalPoints||[]).map(k=>['point',k])].map(([t,k])=>`<button class="ac-check" data-select-type="${t}" data-kind="${k}" title="${escHtml(anyDesc(t,k))}">+ <span class="ac-check-ru">${escHtml(anyRu(t,k))}</span><code class="ac-check-code">${escHtml(k)}</code></button>`).join('') || '<span class="lab-empty">—</span>'}</div></details>
