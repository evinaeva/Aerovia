...
> **⚠️ ОБНОВЛЕНО 2026-06-18 — реализован ЛЁГКИЙ путь (НЕ полный Supabase).** Для масштаба
> «Play-only, пара тестеров → возможно сотни» развёрнут **self-hosted auto-update без Supabase**:
> плагин ходит на свой `updateUrl` (`https://capgo.jevgenia.com/updates`), бандл (zip `www/`)
> раздаётся статикой с `/bundles`, версии сравнивает крошечный responder (Caddy авто-TLS + systemd
> `capgo-ota` на Contabo); CI зипует и заливает бандл на VPS по SSH (`deploy.yml`). Полный стек
> Supabase ниже **НЕ разворачивался** (избыточен для масштаба) — раздел оставлен как справка и
> запасной путь. Откат/поэтапную раскатку доращиваем в responder по мере роста.
