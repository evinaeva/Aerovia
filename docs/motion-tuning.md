...
Параметры извлечены из существующего кода `src/game/04-config-levels.ts`, `src/game/08-gameplay.ts`, `src/game/08b-gameplay-step.ts`, `src/game/09b-render-entities.ts`, `src/game/10-scene-loop.ts`, `src/game/11b-editor.ts` и CSS-анимаций. В первую очередь вынесены параметры, которые уже существуют в `K` и `FOR` и реально читаются игровым циклом.

| Параметр | Где хранится | Значение | Диапазон Motion Tuning | Назначение | Влияние |
|---|---:|---:|---:|---|---|
...
| `K.TAKEOFF_HOLD` | `04-config-levels.ts` | `0.5` | `0..2.5` | пауза перед разгоном | длиннее/короче остановка перед вылетом |
...
### Зашитые в код параметры, пока не редактируемые

* Множители/числа внутри симуляции: landing speed `K.SPEED_AIR * 0.8`, bay movement `taxiSpeed * 0.85`, bonus fly `K.SPEED_TAKEOFF * 0.7`, lerp `dt*6`, `dt*8`, `dt*2`, угловой порог `0.12`, геометрия парковки `L=13*ui`, `gap=4*ui`.
* Визуальные тайминги меню и HUD: CSS `transition:.15s`, `transition:.32s`, `--m-dur`, `starPop 0.5s`.
* Таймеры инфраструктуры: revocation/export `setTimeout(..., 5000)`, achievement toast gap/show, cloud push debounce.

## Этапы 2–7. Реализация

Добавлен единый runtime-реестр `MT_PARAMS`: он описывает ключ, категорию, подпись, исходное значение, диапазон, назначение и влияние параметра. Панель читает/пишет только через этот реестр, поэтому параметры не размазываются по UI-коду.

* Источник данных: `MT_PARAMS` и методы `MT.snapshot/apply/reset/export/importText`.
* Сохранение/загрузка: `localStorage` ключ `pf_motion_tuning_v1`.
* Defaults/reset: исходные значения берутся из `def` в реестре.
* Presets: произвольные пользовательские пресеты в `pf_motion_presets_v1`.
* Export/import: JSON с `version`, `exportedAt`, `values`.
* Live Preview: изменение слайдера сразу мутирует существующие `K`/`FOR`; игровой цикл читает эти объекты каждый кадр, поэтому перезагрузка/сборка не нужны.

## Этап 8. Рекомендации

1. Обязательно держать в Motion Tuning все параметры движения, маршрутизации, таймингов обслуживания, spawn pace, столкновений, near-miss, погодных множителей и лесных бригад.
2. В коде лучше оставить чистую геометрию экрана, safe-area, константы тестовой/системной инфраструктуры, а также art-only размеры, которые привязаны к конкретному skin/handoff.
3. Новые параметры на будущее: множитель посадочного докатывания, скорость заезда/выезда из бокса, lerp alignment speed, порог разворота в боксе, длительность HUD/toast/achievement-анимаций, визуальная толщина/яркость route preview.
4. Архитектура: расширять `MT_PARAMS`, а не добавлять новые слайдеры вручную; если параметр перестаёт быть числом `K`/`FOR`, добавить адаптер target в `mtTarget`.
