...
- Из [`play-featuring-plan.md`](play-featuring-plan.md) — чисто ручные пункты Play Console,
  не связанные с кодом: заполнить **анкету контент-рейтинга**; сверить `targetSdkVersion`
  с текущим минимумом Play; смерить размер AAB против лимита базового модуля **150 МБ**;
  на первом релизе — **staged rollout** (не 100% сразу) + managed publishing.
