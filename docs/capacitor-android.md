...
- `src/game/12f-back-button.ts` — Android hardware/gesture **back** (`@capacitor/app`):
  закрывает/переключает текущий экран вместо дефолтного выхода из приложения (реюзает
  существующие `.m-back`-кнопки). No-op на вебе/PWA. Разбор гайда —
  [`play-featuring-plan.md`](play-featuring-plan.md).
- Crashlytics + Performance Monitoring, network security config (запрет cleartext) —
  gradle-патчи в `setup-android.mjs`, тот же Firebase-проект, что и Analytics.
...
## Подписанный AAB (release signing)

Для закрытого теста и публикации нужен **подписанный AAB**. Каркас подписи уже в
[`setup-android.mjs`](../scripts/setup-android.mjs): при release-сборке `app/build.gradle`
читает `keystore.properties` (gitignored, машинно-локальный) и подписывает бандл. Без этого
файла release остаётся неподписанным (debug/CI не затрагиваются). Секреты в git не попадают
(`.gitignore`: `keystore.properties`, `*.keystore`, `*.jks`).

При **Play App Signing** (дефолт) этот ключ — твой *upload-key*: Google хранит настоящий
ключ подписи приложения, ты грузишь бандлы, подписанные upload-ключом. Держать его надёжно
и вечно (потеря = сброс через поддержку Google).

**Разово — создать upload-keystore и заполнить properties:**

    # 1) сгенерировать keystore (пароли выбираешь сам; JBR keytool на машине овнера)
    & "C:\Program Files\Android\Android Studio\jbr\bin\keytool.exe" `
        -genkeypair -v -keystore planeflow-upload.keystore -alias upload `
        -keyalg RSA -keysize 2048 -validity 10000

    # 2) cp keystore.properties.example keystore.properties  (репо-корень) и заполнить
    #    storeFile (абсолютный путь) / storePassword / keyAlias / keyPassword

**Собрать AAB** (из `main` — гейт Firebase из PR #399 уже смержен):

    npm run build:www
    npx cap add android
    npm run setup:android      # скопирует keystore.properties → android/, включит подпись
    npx cap sync android
    cd android && ./gradlew bundleRelease
    # → app/build/outputs/bundle/release/app-release.aab

Это одновременно закрывает блокер **«Смоук release-сборки с R8»** (release = единственный
build type с R8-шринком): после сборки проверь, что мост видит `Snapshots`/`InstallReferrer`/
`FirebaseAnalytics`/`RemoteConfig`, и что Firebase-сбор стартует только после «Принять».

**Загрузка в Play Console + закрытый трек** (≥12 человек, 14 дней) — вручную в консоли под
твоим аккаунтом: залить AAB на закрытый трек, там же взять **SHA-256** из Play App Signing
(App Integrity) для `assetlinks.json` и второй записи в «Учётные данные» PGS; на первом
релизе — **staged rollout** + managed publishing.
...
- Из [`play-featuring-plan.md`](play-featuring-plan.md) — чисто ручные пункты Play Console,
  не связанные с кодом: заполнить **анкету контент-рейтинга**; на первом релизе —
  **staged rollout** (не 100% сразу) + managed publishing.
...
- Аппаратная/жестовая кнопка «назад» (`12f-back-button.ts`) — из паузы/меню/уровня должна
  переключать экран, а не сразу закрывать приложение; только со startScreen — реальный выход.
  Логика навигации прогнана в браузере (см. `play-featuring-plan.md`), но не на устройстве.
