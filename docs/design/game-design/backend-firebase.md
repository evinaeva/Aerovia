...
> **⚠️ SUPERSEDED в веб/Firebase-части (решение 2026-06-18: распространение ТОЛЬКО через Google Play).**
> Веб-игроков нет → **веб-рейтинг на Firebase + App Check сняты** (вернутся только при веб-дистрибуции).
> Актуально из этого дока: Android-слой = **Play Games Services** (ачивки/XP/лидерборд, код `src/game/12b-native-play-games.ts`).
> Облачные сейвы — **Play Games Saved Games** (реализованы), НЕ Firebase. Серверный анти-чит — отложен.
> Полная картина решения — [`../../backlog.md`](../../backlog.md) и [`../../done.md`](../../done.md).
